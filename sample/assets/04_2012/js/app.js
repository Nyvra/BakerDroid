...
$(function(){
	...
	*/
	...
	//get response-complete if exists
	$('.response-complete').each(function(index) {
		var index = index + 1;
		var key	  = "rc"+lessonId+"-"+index;
		var val	  = window.localStorage.getItem(key);
	...
	//get question-check if exists
	$('.question-check').each(function(index) {
		var index = index + 1;
		var key	  = "ck"+lessonId+"-"+index;
		var val	  = window.localStorage.getItem(key);
	...
	//get question-radio if exists
	
	$('form').each(function(index) {
		var key	= lessonId+".frm-"+index;
		var val	= window.localStorage.getItem(key);	
	
		$(this).attr("rel", key);
		
		if(val != null) {
			$("input", this).eq(val).attr('checked','checked');
		}
	});
	...
	$('.response, .annotations, .response-complete').blur(function(){		
	...
	//Save question-check on localstorage
	...
	//Save question-radio on localstorage
	$('.question-radio').change(function(){
		var name = $(this).attr("name");

		var radioBtn = $('form input:radio[name='+name+']');
		
		var key = $(this).closest('form').attr('rel');
		var val = radioBtn.index(radioBtn.filter(':checked'));

		window.localStorage.setItem(key, val);
	});

	
	//autoresize text-area and input
	$('.response, .annotations').autoResize();
	$('.response-complete').autoResize({extraSpace: 10});
